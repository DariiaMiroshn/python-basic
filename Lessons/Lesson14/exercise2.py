...
class Counter:
   ...
   def set_min(self, min_min):
       if min_min <= self.max_value:
           ...
           raise ValueError ("Min_value cannot be bigger than max_value")

   def step_up(self):
        if self.current < self.max_value:
           self.current += 1
        else:
            raise ValueError ("You reached maximum")

   def step_down(self):
       if self.current > self.min_value:
          self.current -= 1
       else:
           raise ValueError("You reached minimum")
